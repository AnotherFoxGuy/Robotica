...
namespace robotica {
    ...
    inline mazemap generate_map(void) {
        ...
        const static std::array edges = {
            ...
            std::tuple { 1,  2,  maze_vertex::SOUTH, 2300 },
            std::tuple { 1,  3,  maze_vertex::EAST,  3300 },
        ...
        for (const auto& [a, b, dir, dist] : edges) {
            verts[a].edges.push_back({ a, b, dist, dir });
            verts[b].edges.push_back({ b, a, dist, inverse(dir) });
        }

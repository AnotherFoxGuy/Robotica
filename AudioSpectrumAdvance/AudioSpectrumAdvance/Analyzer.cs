...
namespace AudioSpectrumAdvance
{
    internal class Analyzer
    {
        ...
        public Analyzer(ProgressBar left, ProgressBar right, Spectrum spectrum, ComboBox devicelist, Chart chart)
        {
            ...
            for (var i = 0; i < chart.ChartAreas["ChartArea1"].AxisX.Maximum; i++)
            {
            ...
            set
            {
                ...
                if (value)
                {
                    if (!_initialized)
                    {
                        ...
                        var result = BassWasapi.BASS_WASAPI_Init(devindex, 0, 0, BASSWASAPIInit.BASS_WASAPI_BUFFER, 1f,
        ...
        {
            var ret = BassWasapi.BASS_WASAPI_GetData(_fft, (int) BASSData.BASS_DATA_FFT8192); //get channel fft data
            if (ret < -1) return;
            int x, y;
            var b0 = 0;
            ...
            for (var i = 0; i < _spectrumdata.ToArray().Length; i++)
            {
                try
                {
